...
class Client(object):
    ...
    # DOMAIN = "52.68.187.229"
    HOST = "api.binance.com"
    API_URL = 'https://%s/api' % HOST
    WITHDRAW_API_URL = 'https://%s/wapi' % HOST
    ...
    def _generate_signature(self, data):
        ...
        query_string = '&'.join(ordered_data)
    ...
    def _order_params(self, data):
        ...
        for key in sorted(data.keys()):
            if key == 'signature':
                ...
                params.append("{}={}".format(key, data[key]))
        # sort parameters by key
        params.sort(key=itemgetter(0))
        if has_signature:
            params.append('signature={}'.format(data['signature']))
        return params
    ...
    def _request(self, method, uri, signed, force_params=False, **kwargs):
        ...
        if signed:
            ...
            data['signature'] = self._generate_signature(kwargs['data'])

        data_string = ""
        if data and len(data) > 0:
            data_string = '&'.join(self._order_params(data))
    ...
    def _handle_response(self, request, response, status_code):
        ...
        if not status_code//100 == 2:
            raise BinanceAPIException(request, response, status_code)
        try:
            ...
            raise BinanceRequestException('Invalid Response: %s' % response)
