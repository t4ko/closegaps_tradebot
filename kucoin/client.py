...
class Client(object):
    ...
    def _request(self, method, path, signed, **kwargs):
        ...
        headers = ['User-Agent: ',
                   'KC-API-KEY: %s' % self.API_KEY]
        param_headers = kwargs.get('headers', {})
        for header_name in param_headers:
            headers.append("%s: %s" % (header_name, param_headers[header_name]))

        # Prepare the URL
        full_path = '/%s/%s' % (self.API_VERSION, path)
        uri = '%s%s' % (self.API_URL, full_path)

        # Prepare the data
        data = kwargs.get('data', {})
        data_string = ""
        if data and len(data) > 0:
            data_string = self._order_params_for_sig(data)
        if method is 'get':
        ...
        if signed:
            ...
            headers.append('KC-API-NONCE: %s' % str(nonce))
            headers.append('KC-API-SIGNATURE: %s' % self._generate_signature(full_path, data_string, nonce))

        # Apply the settings
        buff = BytesIO()
        session.setopt(pycurl.HTTPHEADER, headers)
        session.setopt(pycurl.URL, uri)
        session.setopt(pycurl.WRITEDATA, buff)
        session.perform()
        return self._handle_response(uri, buff.getvalue(), session.getinfo(pycurl.RESPONSE_CODE))

    def _handle_response(self, request, response, status_code):
        """Internal helper for handling API responses from the Quoine server.
        Raises the appropriate exceptions when necessary; otherwise, returns the
        response.
        """
        if not status_code//100 == 2:
            raise KucoinAPIException(request, response, status_code)
        try:
            ...
            if 'timestamp' in content:
                self._last_timestamp = content['timestamp']

            # if it's a normal response we have a data attribute, return that
            if 'data' in content:
                return content['data']
            else:  # by default return full response
                return content
        except ValueError:
            raise KucoinRequestException('Invalid Response: %s' % response)
